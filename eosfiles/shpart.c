...
int main(int argc, char *argv[]) {
  ...
  if (argc != 2) {
    printf("Usage: %s <disk special file>\n", argv[0]);
    exit(1);
  }
  diskName = argv[1];
  /* read partition table record */
  dskfd = open(diskName, O_RDONLY);
  if (dskfd < 0) {
    error("cannot open disk '%s'", diskName);
  }
  lseek(dskfd, 1 * SECTOR_SIZE, SEEK_SET);
  if (read(dskfd, ptr, SECTOR_SIZE) != SECTOR_SIZE) {
    error("cannot read partition table from disk '%s'", diskName);
  }
  close(dskfd);
